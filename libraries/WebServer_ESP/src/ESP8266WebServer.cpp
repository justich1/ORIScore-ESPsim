...
void ESP8266WebServer::handleClient() {
  // Skutečný fake HTTP server běží na PC ve vlastním vlákně.
}
...
void ESP8266WebServer::handleRawHttpClient(void* socketHandle) {
  ...
  {
    std::lock_guard<std::recursive_mutex> lock(stateMutex);
    code = statusCode == 0 ? 200 : statusCode;
    reason = httpReason(code);
    contentType = responseContentType.length() ? responseContentType : String("text/plain; charset=utf-8");
    bodyOut = response;
