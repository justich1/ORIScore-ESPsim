...
static std::recursive_mutex gIoMutex;

// ------------------------------------------------------------
// Simulovaný ESP heap
// ------------------------------------------------------------
// DŮLEŽITÉ:
// Dříve se tu přepisoval globální operator new/delete, aby se měřily
// všechny C++ alokace. To je u MSVC ve firmware simulátoru rizikové:
// při větším web UI / více vláknech / std::string alokacích může dojít
// k 0xC0000005 (Access violation). Proto je globální heap hook vypnutý.
//
// ESP.getFreeHeap() a ESP.getMaxFreeBlockSize() teď vrací stabilní
// simulované hodnoty podle board JSON / ORISIM_RAM_TOTAL. Až bude potřeba
// skutečné sledování alokací, je lepší ho dělat opt-in uvnitř String
// nebo přes bezpečný allocator, ne globálním přepsáním new/delete.

static std::recursive_mutex gSimHeapConfigMutex;
static unsigned long gSimHeapTotalBytes = 81920;
static unsigned long gSimHeapBaseUsedBytes = 0;

static unsigned long simHeapUsedLocked() {
  return std::min<unsigned long>(gSimHeapBaseUsedBytes, gSimHeapTotalBytes);
}
...
static unsigned long simHeapMaxBlockLocked() {
  ...
  // Bez globálního hooku neznáme reálnou fragmentaci. Necháme bezpečný
  // orientační model: největší blok je cca 90 % volné RAM.
  unsigned long reserve = std::max<unsigned long>(256UL, freeHeap / 10UL);
  return freeHeap > reserve ? freeHeap - reserve : freeHeap;
}
...
static void emitPinState(uint8_t pin) {
  std::lock_guard<std::recursive_mutex> ioLock(gIoMutex);
...
void simVirtualHwSet(const String& typeIn, const String& name, const String& pinIn, const String& addressIn,
  ...
  if (type.startsWith("ANALOG") || type == "MQ135") {
    ...
    if (gpio >= 0 && gpio <= 255) {
      std::lock_guard<std::recursive_mutex> ioLock(gIoMutex);
      gAnalogValues[(uint8_t)gpio] = simParseAnalogRaw(value);
    }
  }
}
...
int simGetPinMode(uint8_t pin) {
  std::lock_guard<std::recursive_mutex> ioLock(gIoMutex);
  auto it = gPinModes.find(pin);
  return it == gPinModes.end() ? -1 : it->second;
}
...
void FakeESPClass::resetHeap(uint32_t freeHeap, uint32_t maxBlock) {
  ...
  unsigned long requestedFree = freeHeap > 0
    ? std::min<unsigned long>((unsigned long)freeHeap, gSimHeapTotalBytes)
    : gSimHeapTotalBytes;

  gSimHeapBaseUsedBytes = gSimHeapTotalBytes > requestedFree
    ? gSimHeapTotalBytes - requestedFree
    : 0;
